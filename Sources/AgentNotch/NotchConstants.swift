...
/// Centralized backend endpoints. All HTTP calls from the notch process go
...
public enum NotchEndpoints {
    ...
    public static var sseStream: URL { URL(string: "\(host)/api/notch/stream")! }
    public static var send: URL { URL(string: "\(host)/api/notch/send")! }
    public static var prefs: URL { URL(string: "\(host)/api/notch/prefs")! }
    public static var barge: URL { URL(string: "\(host)/api/notch/barge")! }
    public static var abort: URL { URL(string: "\(host)/api/notch/abort")! }

    // ─── Orchestrator-provider endpoints (sessions + todos) ────────────────
    /// `GET` returns the list of live AI coding sessions (typically Claude Code).
    public static var localSessions: URL { URL(string: "\(host)/api/local-sessions")! }
    /// `GET /api/todos/<id>` returns one todo, `POST /complete` marks done.
    public static func todo(id: String) -> URL { URL(string: "\(host)/api/todos/\(id)")! }
    public static func todoComplete(id: String) -> URL { URL(string: "\(host)/api/todos/\(id)/complete")! }
    /// Convenience: open the orchestrator web view focused on a specific pid.
    public static func orchestratorWeb(pid: Int) -> URL { URL(string: "\(host)/orchestrator?pid=\(pid)")! }
}
...
enum NotchTuning {
    ...
    /// Minimum gap between successive `interruptBackend` calls (debounce so
