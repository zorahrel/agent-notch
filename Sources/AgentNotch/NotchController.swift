...
/// Owns the single `DynamicNotch` instance that hosts the orb. The
...
final class NotchController: ObservableObject {
    ...
    fileprivate func handleMouseMoved(cgX: CGFloat, cgY: CGFloat) {
        ...
        // Hot-corner is a universal "stop the assistant" gesture — works during
        ...
        if d <= NotchTuning.cornerAbortPx {
            if streamRecorder.isRunning {
                ...
                interruptBackend(reason: "hot-corner")
            }
        }
        if streamRecorder.isRunning {
    ...
    private func startStreamingVoice() {
        ...
        // Wake up Silero VAD so barge-in (interrupting the assistant mid-TTS) and
    ...
    /// flight TTS playback so the user can stop the assistant by flicking the
    ...
    @MainActor
    private func interruptBackend(reason: String) {
        let now = Date().timeIntervalSince1970
