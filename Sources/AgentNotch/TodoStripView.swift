...
/// top-3 open Apple Reminders todos in the configured Reminders list.
///
...
public struct TodoStripView: View {
    ...
    private func reassign(todoId: String, pid: Int) async {
        var req = URLRequest(url: NotchEndpoints.todo(id: todoId))
    ...
    fileprivate static func completeRequest(id: String, session: URLSession) async throws {
        var req = URLRequest(url: NotchEndpoints.todoComplete(id: id))
        req.httpMethod = "POST"
...
private struct SessionPickerView: View {
    ...
    var body: some View {
        ...
        .padding()
        .task {
            if let (data, _) = try? await URLSession.shared.data(from: NotchEndpoints.localSessions),
